...
public class GetTimeAgo extends Application {
    ...
    public static String getTimeAgo(long time, Context ctx,String locale) {
        ...
        if (diff < MINUTE_MILLIS) {
            ...
            if(locale.equals("ar")) return getStringByLocal(ctx,R.string.time_arabic,locale)+" "+diff / MINUTE_MILLIS+" "+getStringByLocal(ctx,R.string.minutes_ago,locale);
            return diff / MINUTE_MILLIS + " "+getStringByLocal(ctx,R.string.minutes_ago,locale);
        } else if (diff < 90 * MINUTE_MILLIS) {
            return getStringByLocal(ctx,R.string.an_hour_ago,locale);
        } else if (diff < 24 * HOUR_MILLIS) {
            if(locale.equals("ar")) return getStringByLocal(ctx,R.string.time_arabic,locale)+" "+diff / HOUR_MILLIS + " "+getStringByLocal(ctx,R.string.hours_ago,locale);
            return diff / HOUR_MILLIS + " "+getStringByLocal(ctx,R.string.hours_ago,locale);
